...
const Waves = () => {
  ...
  const moved = useCallback(
    (point: WavePoint, withCursorForce = true) => {
      const coords = {
        x:
          point.x +
          point.wave.x +
  ...
  useEffect(() => {
    ...
    const setLines = () => {
      ...
      const xStart =
        (width - GRID_X_GAP * totalLines) / 2;

      const yStart =
        (height - GRID_Y_GAP * totalPoints) / 2;
    ...
    const movePoints = (time: number) => {
      ...
      lines.forEach((points) => {
        points.forEach((p) => {
          const noiseInputX =
            (p.x + time * WAVE_TIME_X_FACTOR) *
            WAVE_NOISE_X_FACTOR;

          const noiseInputY =
            (p.y + time * WAVE_TIME_Y_FACTOR) *
            WAVE_NOISE_Y_FACTOR;

          const move =
            noise.perlin2(noiseInputX, noiseInputY) *
            WAVE_NOISE_MAGNITUDE;

          p.wave.x =
            Math.cos(move) * WAVE_AMPLITUDE_X;

          p.wave.y =
            Math.sin(move) * WAVE_AMPLITUDE_Y;
          ...
          if (d < influenceRadius) {
            ...
            const force =
              Math.cos(d * MOUSE_FALLOFF_FACTOR) *
              falloff;
            ...
            p.cursor.vx +=
              Math.cos(mouse.a) * forceFactor;

            p.cursor.vy +=
              Math.sin(mouse.a) * forceFactor;
          }

          p.cursor.vx +=
            (0 - p.cursor.x) * TENSION_STRENGTH;

          p.cursor.vy +=
            (0 - p.cursor.y) * TENSION_STRENGTH;

          p.cursor.vx *= FRICTION;
          p.cursor.vy *= FRICTION;

          p.cursor.x +=
            p.cursor.vx * CURSOR_DISPLACEMENT_STRENGTH;

          p.cursor.y +=
            p.cursor.vy * CURSOR_DISPLACEMENT_STRENGTH;
    ...
    const updateMousePosition = (
      ...
      mouse.x =
        x - state.bounding.left;

      mouse.y =
        y - state.bounding.top;
